...
def main():
    ...
    for i_, e_ in enumerate(edge_list):
        for kk, x_train_split in enumerate(np.split(x_train, 100, axis=0)):
            ...
            np.save(join(folder_dataset, 'exp_4_tr', 'exp_4_dim_%i_split_%i_tr.npy' % (e_, kk)), x_train_4)
            del x_train_4
    ...
    for i_, e_ in enumerate(edge_list):
        for kk, x_test_split in enumerate(np.split(x_test, 100, axis=0)):
            ...
            np.save(join(folder_dataset, 'exp_4_ts', 'exp_4_dim_%i_split_%i_ts.npy' % (e_, kk)), x_test_4)
            del x_test_4
